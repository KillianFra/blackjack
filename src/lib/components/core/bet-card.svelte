...
<div class="flex justify-between gap-4 rounded bg-black/20 p-4 shadow-lg">
	<div class="flex flex-col gap-2">
		<div class="flex flex-col">
            ...
            <div class="flex">
                <p class="text-left text-2xl">{userStore.bet}$</p>
                {#if userStore.bet > 0}
                        <button onclick={userStore.resetBet} aria-label='reset-bet' class="text-md ml-2">
		...
		<div>
			...
			<p class="w-[100px] text-left text-2xl">{userStore.balance}$</p>
		</div>
	</div>
	<div class="flex flex-col items-end justify-between gap-4">
		<Button class="text-md px-6">DEAL</Button>
